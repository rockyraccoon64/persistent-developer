...
class DeveloperStateSlickRepository(db: Database) extends DeveloperStateRepository {
  override def save(id: String, state: DeveloperState)(implicit ec: ExecutionContext): Future[_] = {
    ...
    db.run {
      sqlu"INSERT INTO dev_state (id, state) VALUES ($id, $name) ON CONFLICT (id) DO UPDATE SET state = $name"
    }
  }
